...
function Uppgift1(inX, inY, inZ)
{
  ...
  totalCost = (inX * inY/100 + inZ)
	...
	while (totalCost > 0)
	{
	 return "inX måste vara större än inY";
	}
...
{
	  // använd för att lagra orden som en array
    var wordColumnStr = inX ; // använd för att lagra strängen som ska bli en kolumn av ord
			var wordArray = wordColumnStr.split(" ").join("\n");
...
function Uppgift3(inX, inY)
{
  ...
  //var post = {name : inX, points : inY};
	...
	post.prototype.toString = function postToString() {return "Name : " + this.name + " Points : " + this.points + df};
 ...
 for (var i = 0; i < scoreBoard.length; i++) {
	 if(scoreBoard[i].name == inX)
	 {
		 scoreBoard[i].points = Math.max(scoreBoard[i].points, inY);
	 }
 }
